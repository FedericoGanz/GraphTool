...
# Fixed parameters ----
        ...
        # Flags
        flags_df <- filter(ctry_df, !region %in% c("Aggregates")) %>% select(country, iso2c)
        ...
        empty_datatable_df <- subset(empty_data_df, select=-c(Var_code, Source))
        ...
        # Default parameters
                ...
                # Countries
                def_ctries_ctry <- c("Pakistan")                                                           # Select only one country as analyzed country. For country names check ctry_vec.
                def_ctries_str <- c("Bangladesh", "Egypt, Arab Rep.", "Ethiopia", "India")                 # Select structural countries. No limit. Check that there is no overlapping with analyzed country and aspirational countries. For country names check ctry_vec.
                def_ctries_asp <- c("Indonesia", "Mexico", "Turkey", "Vietnam")                            # Select aspirational countries. No limit. Check that there is no overlapping with analyzed country and structural countries. For country names check ctry_vec.
                def_ctries_reg <- c("South Asia")                                                          # select regions to be included. No limit. For country names check reg_vec.
                def_ctries_all <- FALSE
                
                def_time_range_start <- 2000                                                 # Make sure it makes sense with time slider max and min
                def_time_range_end <- 2019                                                   # Make sure it makes sense with time slider max and min
                def_time_subper <- TRUE                                                      # Logical vector with one element defining whether subperiods will be included
                def_time_subper_num <- 3                                                     # Select between 2, 3 or 4
                def_time_limit_1 <- 2008                                                     # Make sure it makes sense with time_range parameters(start and end) and the other subperiod limits
                def_time_limit_2 <- 2014                                                     # Make sure it makes sense with time_range parameters(start and end) and the other subperiod limits
                def_time_limit_3 <- 2018                                                     # Make sure it makes sense with time_range parameters(start and end) and the other subperiod limits
                def_time_name_1 <- "Historical"
                def_time_name_2 <- "Recent past"
                def_time_name_3 <- "Present"
                def_time_name_4 <- "Period 4"
                
                # Variables                                                                     # Find out variable name and code of the variables you want to select (check var_df)
                def_wdi_vars <- c("GDP growth (annual %) | NY.GDP.MKTP.KD.ZG",
                        "Inflation, consumer prices (annual %) | FP.CPI.TOTL.ZG",
                        "GDP per capita, PPP (current international $) | NY.GDP.PCAP.PP.CD")
                
                # Aesthetic parameters
                def_col_spinner <- c("#009FDA")                                           # Select color. Check options at: https://www.color-hex.com/
                def_col_graphs <- c("#EC7063")                                            # Select color. Check options at: https://www.color-hex.com/
                def_col_graphs_line <- c("#FC4024")                                       # Select color. Check options at: https://www.color-hex.com/
                def_col_palette_mult <- c("Oranges")                                      # Select color. Check options at: https://www.color-hex.com/
                def_selected_font <- c("Calibri")                                            # Select among 4 available fonts: (Calibri = "Calibri", Arial = "sans", Courier = "mono", TimesNewRoman = "serif"). Example: for TimesNewRoman type "serif" instead of "Calibri".
                
                # Calculate auxiliary vectors
                def_ctries_selected <- c(def_ctries_ctry, def_ctries_asp, def_ctries_str, def_ctries_reg)
                def_ctries_selected_iso2c <- ctry_df[ctry_df[ ,"country"] %in% def_ctries_selected, "iso2c"]
                def_wdi_vars_code <- filter(var_df, name_indicator %in% def_wdi_vars) %>% pull(indicator)
                def_time_range <- c(def_time_range_start, def_time_range_end)
...
init_available_str <- ctry_vec[!ctry_vec %in% c(def_ctries_ctry, def_ctries_asp)]
init_available_asp <- ctry_vec[!ctry_vec %in% c(def_ctries_ctry, def_ctries_str)]
                                        ...
                                        sidebarPanel(
                                                        ...
                                                        conditionalPanel(
                                                                ...
                                                                conditionalPanel(
                                                                        ...
                                                                        numericInput(
                                                                                ...
                                                                                min = 2,
                                                                                max = 4,
                                                                                value = def_time_subper_num),
                                                                        
                                                                        textInput(
                                                                                inputId = "in_id_time_name_1",
                                                                                label="Period 1 name",
                                                                                value = def_time_name_1),
                                                                        
                                                                        textInput(
                                                                                inputId = "in_id_time_name_2",
                                                                                label="Period 2 name",
                                                                                value = def_time_name_2),
                                                                        
                                                                        sliderInput(
                                                                                inputId = "in_id_time_limit_1",
                                                                                label = "Select last year of Period 1",
                                                                                sep = "",
                                                                                min = def_time_range_start,
                                                                                max = def_time_range_end,
                                                                                step = 1,
                                                                                value = def_time_limit_1),
                                                ...
                                                h4("Step 5: Select the data sources included in the final dataset"),
                                                        ...
                                                        checkboxGroupInput(
                                                                ...
                                                                choices = def_datasets,
                                                                selected = def_datasets),
                                        ...
                                        sidebarPanel(
                                                ...
                                                h4("Plot parameters"),
                                                        ...
                                                        sliderInput(
                                                                ...
                                                                max = def_time_range_end,
                                                                step = 1,
                                                                value = c(def_time_range_start,def_time_range_end)
                                                        ),
                                                        hr(),
...
server <- function(input, output, session){
        ...
        rv <- reactiveValues(
                ...
                # Countries
                countries_ctry = def_ctries_ctry,
                countries_str = def_ctries_str,
                countries_asp = def_ctries_asp,
                countries_reg = def_ctries_reg,
                countries_all = def_ctries_all,

                # Time
                time_range_start = def_time_range_start,
                time_range_end = def_time_range_end,
                time_subper = def_time_subper,
                time_subper_num = def_time_subper_num,
                time_limit_1 = def_time_limit_1,
                time_limit_2 = def_time_limit_2,
                time_limit_3 = def_time_limit_3,
                time_name_1 = def_time_name_1,
                time_name_2 = def_time_name_2,
                time_name_3 = def_time_name_3,
                time_name_4 = def_time_name_4,

                # Variables
                wdi_vars = def_wdi_vars,

                # Aesthetic parameters
                ae_col_spinner = def_col_spinner,
                ae_col_graphs = def_col_graphs,
                ae_col_graphs_line = def_col_graphs_line,
                ae_col_palette_mult = def_col_palette_mult,
                ae_selected_font = def_selected_font,

                # Auxiliary vectors
                countries_selected = def_ctries_selected,
                countries_selected_iso2c = def_ctries_selected_iso2c,
                wdi_vars_code = def_wdi_vars_code,
                time_range = def_time_range,
                
                # Datasets
                datasets = def_datasets,
                remove_dup = def_remove_dup,
                remove_na = def_remove_dup,
                
                # Show/hide tables
                show_table = TRUE,
                
                # Dataframes
                df_wdi = empty_data_df,
                df_external_last = empty_data_df %>% select(),
                df_external_all = empty_data_df,
                df_all_data_raw = empty_data_df,
                df_all_data = empty_datatable_df,
                df_metadata = empty_metadata
        ...
        # SL.1. Data tab ----
                ...
                # SL.1.1. Inputs and data processing ----
                        ...
                        ## SL.1.1.1. Countries choice: update countries available in menus (UI.1.1.1.) ====
                                ...
                                fun_listen_ctries <- reactive({
                                        no_changes <- as.logical(c(
                                                setequal(rv$countries_ctry, input$in_id_ctries_ctry),
                                                setequal(rv$countries_str, input$in_id_ctries_str),
                                                setequal(rv$countries_asp, input$in_id_ctries_asp),
                                                setequal(rv$countries_reg, input$in_id_ctries_reg),
                                                setequal(rv$countries_all, input$in_id_ctries_all)
                                                ))
                                        return(all(no_changes))
                                })
                                
                                # Update structural countries input menu
                                observeEvent(fun_listen_ctries(), {
                                        changes <- !fun_listen_ctries()
                                        if(changes){
                                                print("Updating input menu: data/inputs/countries/structural")
                                                x <- ctry_vec[!ctry_vec %in% c(input$in_id_ctries_ctry, input$in_id_ctries_asp)]
                                ...
                                observeEvent(fun_listen_ctries(), {
                                        ...
                                        if(changes){
                                                ...
                                                x <- ctry_vec[!ctry_vec %in% c(input$in_id_ctries_ctry , input$in_id_ctries_str)]
                                                updateSelectizeInput(
                                                        inputId = 'in_id_ctries_asp',
                                                        choices = x,
                                ...
                                observeEvent(fun_listen_ctries(), {
                                        ...
                                        if(changes){
                                                ...
                                                shinyjs::show("in_id_ctries_update")
                                        } else {
                                                shinyjs::hide("in_id_ctries_update")
                                        }
                                }, ignoreInit = TRUE)
                                
                                observeEvent(input$in_id_ctries_update, {
                                        rv$countries_ctry <- input$in_id_ctries_ctry
                                        rv$countries_str <- input$in_id_ctries_str
                                        rv$countries_asp <- input$in_id_ctries_asp
                                        rv$countries_reg <- input$in_id_ctries_reg
                                        rv$countries_all <- input$in_id_ctries_all
                        ...
                        ## SL.1.1.2. Update: Subperiods (UI.1.1.2.) ====
                                ...
                                fun_listen_time_range <- reactive({
                                        no_changes <- as.logical(c(
                                                setequal(rv$time_range, input$in_id_time_range)
                                        ))
                                        return(all(no_changes))
                                        ...
                                        if("in_id_time_subper_num" %in% names(input)){
                                                if (input$in_id_time_subper_num >= 3) {
                                                        ...
                                                        if("in_id_time_limit_2" %in% names(input)){
                                                                ...
                                                                in_id_time_limit_2_original <- rv$time_limit_2
                                                        }
                                        ...
                                        if("in_id_time_subper_num" %in% names(input)){
                                                ...
                                                if(input$in_id_time_subper_num == 4) {
                                                        ...
                                                        if("in_id_time_limit_3" %in% names(input)){
                                                                ...
                                                                in_id_time_limit_3_original <- rv$time_limit_3
                                                        }
                                ...
                                fun_listen_time <- reactive({
                                        ...
                                        if(input$in_id_time_subper == FALSE){                                        
                                                no_changes <- as.logical(c(
                                                        setequal(rv$time_range, input$in_id_time_range),
                                                        setequal(rv$time_subper, input$in_id_time_subper)
                                                        # setequal(rv$time_subper_num, input$in_id_time_subper_num)
                                                        # setequal(rv$time_name_1, input$in_id_time_name_1)
                                                        # setequal(rv$time_name_2, input$in_id_time_name_2)
                                                        # setequal(rv$time_name_3, input$in_id_time_name_3)
                                                        # setequal(rv$time_name_4, input$in_id_time_name_4)
                                                        # setequal(rv$time_limit_1, input$in_id_time_limit_1)
                                                        # setequal(rv$time_limit_2, input$in_id_time_limit_2)
                                                        # setequal(rv$time_limit_3, input$in_id_time_limit_3)
                                                ))
                                        }                                        
                                ...
                                observeEvent(input$in_id_time_update, {
                                        rv$time_range <- input$in_id_time_range
                                        rv$time_subper <- input$in_id_time_subper
                                        rv$time_subper_num <- as.numeric(input$in_id_time_subper_num)
                                        rv$time_name_1 <- input$in_id_time_name_1
                                        rv$time_name_2 <- input$in_id_time_name_2
                                        rv$time_name_3 <- input$in_id_time_name_3
                                        rv$time_name_4 <- input$in_id_time_name_4
                                        rv$time_limit_1 <- as.numeric(input$in_id_time_limit_1)
                                        rv$time_limit_2 <- as.numeric(input$in_id_time_limit_2)
                                        rv$time_limit_3 <- as.numeric(input$in_id_time_limit_3)
                        ...
                        observeEvent(input$in_id_ctries_update, {
                                ...
                                rv$countries_selected_iso2c <- ctry_df[ctry_df[,"country"] %in% rv$countries_selected, "iso2c"]
                                
                                # Condition: if all countries selected, set all
                                if(input$in_id_ctries_all){
                                        rv$countries_selected_iso2c <- c("all")
                                        rv$countries_selected <- c(ctry_vec, reg_vec)
                                }
                        }, ignoreInit = TRUE)
                                
                        # Update WDI search parameters: time
                        observeEvent(input$in_id_time_update, {
                                rv$time_range_start <- as.numeric(min(input$in_id_time_range, isolate(input$in_id_time_range)))
                                rv$time_range_end <- as.numeric(max(input$in_id_time_range, isolate(input$in_id_time_range)))
                        }, ignoreInit = TRUE)
                        
                        # Update WDI search parameters: variables
                        observeEvent(input$in_id_vars_update, {
                                rv$wdi_vars_code <- filter(var_df, name_indicator %in% input$in_id_wdi_vars) %>% pull(indicator)
                        }, ignoreInit = TRUE)
                        ...
                        observeEvent(fun_listen_wdi_params(),{
                                ...
                                print(rv$countries_selected)
                                print("(2/3) Selected variables:")
                                print(rv$wdi_vars_code)
                                print("(3/3) Year range:")
                                print(rv$time_range)

                                # Condition: if selected countries or variables empty - stop
                                if(length(rv$countries_selected)==0 | length(rv$time_range)==0 | length(rv$wdi_vars_code)==0){
                                        print("A WDI download parameter (countries or variable) is missing: no WDI data download (rv$df_wdi empty)")
                                        return(rv$df_wdi <- data.frame())
                                }
                                
                                # Data download
                                print("All required WDI parameters are loaded: beginning WDI data download... (SL.1.2.)")
                                raw_wdi <- WDI(
                                        country = rv$countries_selected_iso2c,
                                        indicator = rv$wdi_vars_code,
                                        start = rv$time_range_start,
                                        end = rv$time_range_end,
                                        extra = FALSE,
                                        cache = NULL,
                                        latest = NULL,
                                        language = "en")
                                ...
                                vars_downloaded <- rv$wdi_vars_code[rv$wdi_vars_code %in% names(raw_wdi)]                
                                vars_not_downloaded <- rv$wdi_vars_code[!rv$wdi_vars_code %in% names(raw_wdi)]
                                ...
                                # Data processing
                                        ...
                                        # Create a dataset with num_ctries * num_years rows
                                        # that includes only variable "country" and year
                                        num_years <- rv$time_range_end - rv$time_range_start + 1
                                        proc_wdi <- rep(rv$countries_selected, each = num_years)
                                        proc_wdi <- data.frame(proc_wdi)
                                        names(proc_wdi)[names(proc_wdi) == "proc_wdi"] <- "country"
                                        proc_wdi$year <- rv$time_range_start:rv$time_range_end
                                        ...
                                        proc_wdi$Ctry_group <- ifelse(
                                                proc_wdi$country %in% rv$countries_ctry , "Analized", 
                        ...
                        observeEvent(input$in_id_external,{
                                ...
                                if(length(rv$countries_selected) == 0 | length(rv$time_range) == 0){
                                        print("A parameter (countries or time period) is missing: no WDI data download (rv$df_wdi empty)")
                                        return()
                                }
                                ...
                                # that includes only variable "country" and year
                                        ...
                                        # Years
                                        years <- rv$time_range_start:rv$time_range_end
                                        num_years <- length(years)
                                        
                                        # Countries
                                        ctry_external <- data.frame(rep(rv$countries_selected, each = num_years))
                                        ...
                                        Year <- rep(years, length(rv$countries_selected))
                                        ...
                                        aux_external_vars <- rbind(vars_external, vars_external[rep(1:nrow(vars_external), num_ctries-1), ])
                        ...
                        to_listen_in_id_external_m1 <- reactive({
                                empty_ctries <- as.logical(c(
                                        length(input$in_id_ctries_ctry) == 0,
                                        length(input$in_id_ctries_asp) == 0,
                                        length(input$in_id_ctries_str) == 0,
                                        length(input$in_id_ctries_reg) == 0
                                ))
                                return(all(empty_ctries) & input$in_id_ctries_all == FALSE)
                        })
                        ...
                        observeEvent(input$in_id_external_update,{
                                ...
                                rv$datasets <- append(rv$datasets, input$in_id_external$name)
                        ...
                        ## SL.1.1.9. Merge all data uploaded (WDI and external) into a single dataframe and filter applying country and range criteria ====
                                to_listen_data <- reactive({
                                        ...
                                        print("At least one input parameter that defines rv$df_all_data has been introduced/modified...")

                                        list(
                                                input$in_id_ctries_update[1],
                        ...
                        observeEvent(to_listen_data(),{
                                ...
                                print("Merging rv$df_wdi and rv$external_all (they can be empty) into a single dataframe (rv$df_all_data)...")
                                
                                # Upload 
                                rv$df_all_data <- rbind.fill(rv$df_wdi, rv$external_all)
                                
                                if(empty(rv$df_all_data)){return(print("Dataframe rv$df_all_data is empty"))}
                                
                                # Apply country and year filter
                                rv$df_all_data <- rv$df_all_data %>% filter( 
                                        Year >= rv$time_range_start & Year <= rv$time_range_end &
                                        Country %in% rv$countries_selected &
                                        Source %in% input$in_id_datasets)
                                
                                # Order data
                                rv$df_all_data <- rv$df_all_data[order(rv$df_all_data$Var_name,
                                        rv$df_all_data$Var_code,
                                        rv$df_all_data$Ctry_group_num,
                                        rv$df_all_data$Ctry_iso,
                                        rv$df_all_data$Year),]
                                
                                # Include year periods
                                if(rv$time_range[2]-rv$time_range[1]>=2){
                                        if(rv$time_subper){
                                                if(rv$time_subper_num == 2){
                                                        rv$df_all_data$Period[rv$df_all_data$Year <= rv$time_limit_1] <- rv$time_name_1
                                                        rv$df_all_data$Period_num[rv$df_all_data$Year <= rv$time_limit_1] <- 1
                                                        rv$df_all_data$Period[rv$df_all_data$Year > rv$time_limit_1] <- rv$time_name_2
                                                        rv$df_all_data$Period_num[rv$df_all_data$Year > rv$time_limit_1] <- 2
                                                        
                                                }
                                                if(rv$time_subper_num == 3){
                                                        rv$df_all_data$Period[rv$df_all_data$Year <= rv$time_limit_1] <- rv$time_name_1
                                                        rv$df_all_data$Period_num[rv$df_all_data$Year <= rv$time_limit_1] <- 1
                                                        rv$df_all_data$Period[rv$df_all_data$Year > rv$time_limit_1 & 
                                                                        rv$df_all_data$Year <= rv$time_limit_2] <- rv$time_name_2
                                                        rv$df_all_data$Period_num[rv$df_all_data$Year > rv$time_limit_1 & 
                                                                        rv$df_all_data$Year <= rv$time_limit_2] <- 2
                                                        rv$df_all_data$Period[rv$df_all_data$Year > rv$time_limit_2] <- rv$time_name_3
                                                        rv$df_all_data$Period_num[rv$df_all_data$Year > rv$time_limit_2] <- 3
                                                }
                                                if(rv$time_subper_num == 4){
                                                        rv$df_all_data$Period[rv$df_all_data$Year <= rv$time_limit_1] <- rv$time_name_1
                                                        rv$df_all_data$Period_num[rv$df_all_data$Year <= rv$time_limit_1] <- 1
                                                        rv$df_all_data$Period[rv$df_all_data$Year > rv$time_limit_1 & 
                                                                        rv$df_all_data$Year <= rv$time_limit_2] <- rv$time_name_2
                                                        rv$df_all_data$Period_num[rv$df_all_data$Year > rv$time_limit_1 & 
                                                                        rv$df_all_data$Year <= rv$time_limit_2] <- 2
                                                        rv$df_all_data$Period[rv$df_all_data$Year > rv$time_limit_2 & 
                                                                        rv$df_all_data$Year <= rv$time_limit_3] <- rv$time_name_3
                                                        rv$df_all_data$Period_num[rv$df_all_data$Year > rv$time_limit_2 & 
                                                                        rv$df_all_data$Year <= rv$time_limit_3] <- 3
                                                        rv$df_all_data$Period[rv$df_all_data$Year > rv$time_limit_3 ] <- rv$time_name_4
                                                        rv$df_all_data$Period_num[rv$df_all_data$Year > rv$time_limit_3 ] <- 4
                                                }
                                        }
                                }
                                ...
                                rv$df_all_data <- subset(rv$df_all_data, select=-c(Ctry_group_num, Period_num, Source))
                                
                                # Eliminate duplicated rows
                                if(input$in_id_remove_dup){
                                        rv$df_all_data <- rv$df_all_data %>% distinct()
                                }
                        ...
                        observeEvent(input$in_id_reset_confirmation, {
                                if(input$in_id_reset_confirmation){
                                        ...
                                        # Countries
                                        rv$countries_ctry <- character(0)
                                        rv$countries_asp <- character(0)
                                        rv$countries_str <- character(0)
                                        rv$countries_reg <- character(0)
                                        rv$countries_all <- FALSE
                                        
                                        # Time
                                        rv$time_range_start <- 2000         # Make sure it makes sense with time slider max and min
                                        rv$time_range_end <- 2019           # Make sure it makes sense with time slider max and min
                                        rv$time_subper <- FALSE             # Logical vector with one element defining whether subperiods will be included
                                        rv$time_subper_num <- 2             # Select between 2, 3 or 4
                                        rv$time_limit_1 <- 2008             # Make sure it makes sense with time_range parameters(start and end) and the other subperiod limits
                                        rv$time_limit_2 <- 2014             # Make sure it makes sense with time_range parameters(start and end) and the other subperiod limits
                                        rv$time_limit_3 <- 2018
                                        rv$time_name_1 <- character(0)
                                        rv$time_name_2 <- character(0)
                                        rv$time_name_3 <- character(0)
                                        rv$time_name_4 <- character(0)
                                        
                                        # Variables
                                        rv$wdi_vars <- character(0)
                                        
                                        # Datasets
                                        rv$datasets <- def_datasets
                                        rv$remove_dup <- TRUE
                                        rv$remove_na <- FALSE
                                        ...
                                        # Data
                                        rv$df_wdi <- empty_data_df
                                        rv$external_last <- empty_data_df %>% select()
                                        rv$external_all <- empty_data_df
                                        rv$df_all_data_raw <- empty_data_df
                                        rv$df_all_data <- empty_datatable_df
                                        ...
                                        rv$countries_selected_iso2c <- character(0)
                                        rv$wdi_vars_code <- character(0)
                                        rv$time_range <- c(def_time_range_start, def_time_range_end)
                                        ...
                                        # x <- ctry_vec[!ctry_vec %in% c(rv$countries_ctry, rv$countries_asp)]
                                        updateSelectizeInput(
                                                inputId = 'in_id_ctries_str',
                                                choices = ctry_vec,
                                                selected = c(""))
                                        
                                        # x <- ctry_vec[!ctry_vec %in% c(rv$countries_ctry, rv$countries_str)]
                                        updateSelectizeInput(
                                                inputId = 'in_id_ctries_asp',
                                                choices = ctry_vec,
                                                selected =  c(""))
        ...
        # SL.1.2. Main panel (data presentation) ---- 
                ...
                ## SL.1.2.1. Present data table, download data handler and error messages ====
                        ...
                        output$out_data_table <- renderDataTable({
                                x <- get_data()
                                if (empty(x)){
                                        print("Rendering table: empty rv$df_all_data, no table rendered")
                                        return()} else {if(input$in_id_show_table) {
                                                print("Rendering table: showing rv$df_all_data")
                                                datatable(x,
                                                        rownames = FALSE,
                        ...
                        output$out_download_data <- downloadHandler(
                                print("Creating downloadHandler for rv$df_all_data..."),
                                filename = function() {
                                        paste('data', '.csv', sep='')
                                },
                                content = function(file) {
                                        write.csv(rv$df_all_data, file, row.names = FALSE)
                                }
                        )
                        ...
                        output$out_query_message_data <- renderUI({
                                ...
                                # No data downloaded
                                if(empty(rv$df_all_data)){
                                        print("Printing error message (empty rv$df_all_data): No data has been loaded.")
                                ...
                                if(sum(is.na(rv$df_all_data$Value)) == nrow(rv$df_all_data)) {
                        ...
                        output$out_missing_vars_message_data <- renderUI({
                                to_listen_data()
                                if(!empty(rv$df_all_data)){
                                        vars_not_included <- rv$wdi_vars_code[!rv$wdi_vars_code 
                                                %in% rv$df_wdi$Var_code]
                ...
                # SL.1.2.2. Present metadata table, download metadata handler and error messages ====
                        ...
                        observeEvent(to_listen_data(),{
                                ...
                                print("Creating metadata dataframe (rv$metadata)...")
                
                                # No data downloaded
                                if(is.null(rv$df_all_data)){
                                        print("Dataframe rv$df_all_data is null: empty metadata.")
                                        return()
                                }
                                
                                if(sum(is.na(rv$df_all_data$Value)) == nrow(rv$df_all_data)){
                                        print("Dataframe rv$df_all_data has no Values: empty metadata.")
                                        return()
                                }
                                ...
                                if(!empty(rv$df_wdi)){
                                        ...
                                        metadata_table_wdi <- var_df %>% 
                                                filter(indicator %in% rv$df_wdi$Var_code)
                                        ...
                                        rv$metadata <- metadata_table_wdi
                                        print("Metadata for WDI variables was successfully uploaded to Global Environment.")
                                }
                                
                                # Uploaded files metadata
                                if(!empty(rv$external_all)){
                                        print("Fetching metadata for variables uploaded using external files...")
                                        headers <- names(rv$external_all) %in% c("Var_name", "Var_code", "Description", "Units", "Source_data", "Source_org", "Database", "Source")
                                        metadata_table_uploaded <- rv$external_all[ , headers]
                                        metadata_table_uploaded <- metadata_table_uploaded %>% distinct()
                                        rv$metadata <- rbind.fill(rv$metadata, metadata_table_uploaded)
                                ...
                                rv$metadata <- subset(rv$metadata, 
                                        select=-c(Source))
                        ...
                        output$out_metadata_table <- renderDataTable({
                                to_listen_data()
                                if(input$in_id_show_table & !empty(rv$metadata)) {
                                        print("Rendering metadata table...")
                                        datatable(rv$metadata, rownames = FALSE, options = list(scrollX = T))
                                }
                        })
                        ...
                        output$out_query_result_metadata <- renderUI({
                                ...
                                if(empty(rv$df_all_data)){
                                        print("Printing error message in Metadata tab (empty rv$df_all_data): No data has been loaded.")
                                        return(p("No data has been loaded.", 
                                                style = "color:red")
                                        )
                                }
                ...
                output$out_summary <- renderUI({
                        ...
                        if(empty(rv$df_all_data)){
                                ...
                                text_num_rows <- nrow(rv$df_all_data)
                                text1 <- paste("Number of rows included in final data: ", 
                                        text_num_rows, 
                                        collapse ="")
                                
                                text_num_del_rows <- nrow(rv$df_all_data_raw) - nrow(rv$df_all_data)
                                if(nrow(rv$df_all_data_raw) - nrow(rv$df_all_data) == 0){text_num_del_rows<-"0"}
                                text2 <- paste("Number of rows deleted: ", 
                                        text_num_del_rows, 
                                        collapse ="")
                                
                                text_num_datasets <- length(input$in_id_datasets)
                                if(length(rv$datasets)==0){text_num_datasets<-"0"}
                                text3 <- paste("Number of datasets included in final data: ", 
                                        text_num_datasets, 
                                        collapse ="")
                                
                                text_num_variables <- length(unique(rv$df_all_data$Var_code))
                                if(length(unique(rv$df_all_data$Var_code))==0){text_num_variables<- "0"}
                                text4 <- paste("Number of variables included in final data: ", 
                                        text_num_variables, 
                                        collapse ="")
                                
                                text_num_ctries <- if(input$in_id_ctries_all) {
                                        length(ctry_vec)+length(input$in_id_ctries_reg)} else{
                                        length(rv$countries_selected_iso2c)
                                        }
        ...
        # SL.2. Single Country Bar Plots tab ----
                ...
                # SL.2.1. Inputs and data processing ----
                        ...
                        observeEvent(to_listen_data(),{
                                x <- unique(paste(rv$df_all_data$Var_name, rv$df_all_data$Var_code, sep=" | "))
                        ...
                        output$message_select_vars <- renderUI(
                                if(is.null(rv$df_all_data$Var_name)){helpText("Upload data using Load Data tab")}
                        )
                        ...
                        observeEvent(to_listen_data(),{
                                ...
                                x <- unique(rv$df_all_data$Country)
                                x <- x[!x %in% reg_vec]
                                
                                y <- filter(rv$df_all_data, Ctry_group %in% c("Analized")) %>% pull(Country)
                                y <- unique(y)
                                
                                
                                # Include group in flags
                                
                                flags_df_bar <- filter(flags_df, country %in% x)
                                flags_df_bar <- flags_df_bar[match(x, flags_df_bar$country),]
                                
                                flags_df_bar$Ctry_group <- ifelse(
                                        flags_df_bar$country %in% rv$countries_ctry , "Analized", 
                                        ifelse(flags_df_bar$country %in% rv$countries_str, "Structural", 
                                                ifelse(flags_df_bar$country %in% rv$countries_asp, "Aspirational", 
                                                        ifelse(flags_df_bar$country %in% rv$countries_reg, "Region",
                                                                ifelse((!flags_df_bar$country %in% rv$countries_reg) & (flags_df_bar$country %in% reg_vec), "Rest (region)", "Rest" )
                                                        )
                                                )
                                        )
                                )
                                
                                flags_df_bar$Ctry_Ctry_group <- paste(flags_df_bar$country, flags_df_bar$Ctry_group, sep=" | ")
                                
                                updatePickerInput(
                                                inputId = "gr_bar_id_ctries_ctry",
                                                label = "Select country of analysis",
                                                session = session,
                                                choices = flags_df_bar$country,
                                                choicesOpt = list(content =  
                                                                mapply(flags_df_bar$Ctry_Ctry_group, flags_df_bar$URL, FUN = function(country, flagUrl) {
                                                                        HTML(paste(
                                                                                tags$img(src = flagUrl, width = 20, height = 15),
                                                                                country
                                                                        ))
                        ...
                        observeEvent(input$in_id_time_update,{
                                updateSliderInput(
                                        session= session,
                                        inputId = "gr_bar_id_time_range",
                                        min = rv$time_range_start,
                                        max = rv$time_range_end,
                                        step = 1,
                                        value = rv$time_range)}
                                )
                ...
                # SL.2.2. Create plots ----
                        ...
                        prepped_data_bar <- reactive({
                                ...
                                if(!is.null(input$gr_bar_id_vars)){
                                        ...
                                        x <- rv$df_all_data %>%
                                                as_tibble() %>%
                                                filter(Country == input$gr_bar_id_ctries_ctry & 
                        ...
                        createUI_bar <- function(table) {
                                ...
                                plot_name <- paste0("plot_bar_", as.character(counter_bar))
                                ...
                                if(rv$time_subper & input$gr_bar_id_time_subper){
                                        
                                        # If 2 subperiods, then there are 3 cases
                                        if(rv$time_subper_num == 2){

                                                # Case 1 
                                                if(input$gr_bar_id_time_range[1] < rv$time_limit_1 & input$gr_bar_id_time_range[2] > rv$time_limit_1){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_bar_id_time_range[1]-0.5, rv$time_limit_1+0.5), 
                                                                Year_end=c(rv$time_limit_1+0.5, input$gr_bar_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_1, rv$time_name_2)
                                                        )
                                                }
                                                # Case 2 
                                                if(input$gr_bar_id_time_range[2] <= rv$time_limit_1){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_bar_id_time_range[1]-0.5), 
                                                                Year_end=c(input$gr_bar_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_1)
                                                        )
                                                }
                                                # Case 3
                                                if(input$gr_bar_id_time_range[1] >= rv$time_limit_1){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_bar_id_time_range[1]-0.5), 
                                                                Year_end=c(input$gr_bar_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_2)
                                                        )
                                                }
                                        }
                                        ...
                                        if(rv$time_subper_num == 3){
                                                ...
                                                # Case 1
                                                if(input$gr_bar_id_time_range[1] < rv$time_limit_1 & input$gr_bar_id_time_range[2] > rv$time_limit_2){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_bar_id_time_range[1]-0.5, rv$time_limit_1+0.5, rv$time_limit_2+0.5), 
                                                                Year_end=c(rv$time_limit_1+0.5, rv$time_limit_2+0.5,input$gr_bar_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_1, rv$time_name_2, rv$time_name_3)
                                                        )
                                                }
                                                # Case 2
                                                if(input$gr_bar_id_time_range[1] < rv$time_limit_1 & input$gr_bar_id_time_range[2] <= rv$time_limit_2 & input$gr_bar_id_time_range[2] > rv$time_limit_1){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_bar_id_time_range[1]-0.5, rv$time_limit_1+0.5), 
                                                                Year_end=c(rv$time_limit_1+0.5, input$gr_bar_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_1, rv$time_name_2)
                                                ...
                                                # Case 3
                                                if(input$gr_bar_id_time_range[1] >= rv$time_limit_1 & input$gr_bar_id_time_range[1] < rv$time_limit_2 & input$gr_bar_id_time_range[2] > rv$time_limit_2){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_bar_id_time_range[1]-0.5, rv$time_limit_2+0.5), 
                                                                Year_end=c(rv$time_limit_2+0.5, input$gr_bar_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_2, rv$time_name_3)
                                                        )
                                                }
                                                # Case 4
                                                if(input$gr_bar_id_time_range[2] <= rv$time_limit_1){
                                                ...
                                                # Case 6
                                                if(input$gr_bar_id_time_range[1] >= rv$time_limit_2){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_bar_id_time_range[1]-0.5), 
                                                                Year_end=c(input$gr_bar_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_3)
                                                        )
                                                }
                                        }
                                        ...
                                        if(rv$time_subper_num == 4){
                                                # Case 1
                                                if(input$gr_bar_id_time_range[1] < rv$time_limit_1 & input$gr_bar_id_time_range[2] > rv$time_limit_3){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_bar_id_time_range[1]-0.5, rv$time_limit_1+0.5, rv$time_limit_2+0.5, rv$time_limit_3+0.5), 
                                                                Year_end=c(rv$time_limit_1+0.5, rv$time_limit_2+0.5, rv$time_limit_3+0.5,input$gr_bar_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_1, rv$time_name_2, rv$time_name_3, rv$time_name_4)
                                                        )
                                                }
                                                # Case 2
                                                if(input$gr_bar_id_time_range[1] < rv$time_limit_1 & input$gr_bar_id_time_range[2] <= rv$time_limit_3 & input$gr_bar_id_time_range[2] > rv$time_limit_2){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_bar_id_time_range[1]-0.5, rv$time_limit_1+0.5, rv$time_limit_2+0.5), 
                                                                Year_end=c(rv$time_limit_1+0.5, rv$time_limit_2+0.5,input$gr_bar_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_1, rv$time_name_2, rv$time_name_3)
                                                ...
                                                # Case 3
                                                if(input$gr_bar_id_time_range[1] >= input$in_id_time_limit_1 & input$gr_bar_id_time_range[1] < rv$time_limit_2 & input$gr_bar_id_time_range[2] > rv$time_limit_3){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_bar_id_time_range[1]-0.5, rv$time_limit_2+0.5, rv$time_limit_3+0.5), 
                                                                Year_end=c(rv$time_limit_2+0.5, rv$time_limit_3+0.5, input$gr_bar_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_2, rv$time_name_3, rv$time_name_4)
                                                        )
                                                }
                                                # Case 4
                                                if(input$gr_bar_id_time_range[1] < rv$time_limit_1 & input$gr_bar_id_time_range[2] <= rv$time_limit_2 & input$gr_bar_id_time_range[2] > rv$time_limit_1){
                                                ...
                                                # Case 5
                                                if(input$gr_bar_id_time_range[1] >= rv$time_limit_1 & input$gr_bar_id_time_range[1] < rv$time_limit_2 & input$gr_bar_id_time_range[2] <= rv$time_limit_3 & input$gr_bar_id_time_range[2] > rv$time_limit_2){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_bar_id_time_range[1]-0.5, rv$time_limit_2+0.5), 
                                                                Year_end=c(rv$time_limit_2+0.5, input$gr_bar_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_2, rv$time_name_3)
                                                ...
                                                # Case 6
                                                if(input$gr_bar_id_time_range[1] >= rv$time_limit_2 & input$gr_bar_id_time_range[1] < rv$time_limit_3 & input$gr_bar_id_time_range[2] > rv$time_limit_3){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_bar_id_time_range[1]-0.5, rv$time_limit_3+0.5), 
                                                                Year_end=c(rv$time_limit_3+0.5, input$gr_bar_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_3, rv$time_name_4)
                                                        )
                                                }
                                                # Case 7
                                                if(input$gr_bar_id_time_range[2] <= rv$time_limit_1){
                                                ...
                                                # Case 9
                                                if(input$gr_bar_id_time_range[1] >= rv$time_limit_2 & input$gr_bar_id_time_range[2] <= rv$time_limit_3){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_bar_id_time_range[1]-0.5), 
                                                                Year_end=c(input$gr_bar_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_3)
                                                ...
                                                # Case 10
                                                if(input$gr_bar_id_time_range[1] >= rv$time_limit_3){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_bar_id_time_range[1]-0.5), 
                                                                Year_end=c(input$gr_bar_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_4)
                                                        )
                                                }
                                        }
                                ...
                                p <- ggplot(table, aes(x = Year, y = Value)) +
                                        ...
                                        if(rv$time_subper & input$gr_bar_id_time_subper & !all(is.na(table$Value))){
                                                p <- p + 
        ...
        # SL.3. Multiple Countries Bar Plot tab ----
                ...
                # SL.3.1. Inputs and data processing ---- 
                        ...
                        observeEvent(to_listen_data(),{
                                to_listen_data()
                                z <- unique(paste(rv$df_all_data$Country, rv$df_all_data$Ctry_group, sep=" | "))
                                datfra <- data.frame(z)
                                datfra <- separate(data = datfra,
                                        col = z,
                                        into = c("A", "B"),
                                ...
                                names(ctry_list) <- z
                                y <- c(rv$countries_ctry, rv$countries_asp, rv$countries_str, rv$countries_reg)
                        ...
                        output$gr_mult_message_select_ctries <- renderUI(
                                if(is.null(rv$df_all_data$Country)){helpText("Upload data using Load Data tab")}
                        )                        
                        ...
                        output$gr_mult_id_color_ui <- renderUI({
                                pickerInput(
                                        ...
                                        choices = palette_vec,
                                        selected = rv$ae_col_palette_mult,
                                        multiple = FALSE
                                ) %>%
                                        shinyInput_label_embed(
                                                shiny_iconlink() %>%
                        ...
                        color_reactive <- eventReactive(input$gr_mult_id_color_pal,{
                                rv$ae_col_palette_mult <- input$gr_mult_id_color_pal
                                }
                        )
                ...
                # SL.3.2. Create plots ----
                        ...
                        prepped_data_mult <- reactive({
                                ...
                                if(!is.null(input$gr_mult_id_vars)){
                                        ...
                                        rv$df_all_data_raw %>%
                                                as_tibble() %>%
                                                filter(Country %in% input$gr_mult_id_ctries &
                        ...
                        createUI_mult <- function(table) {
                                ...
                                plot_name <- paste0("plot_mult_", as.character(counter_mult))
                                ...
                                if(input$gr_mult_id_time_subper){
                                        # Period 1
                                        if(input$gr_mult_id_time_range[1] > rv$time_range_start){
                                                x <- paste(rv$time_name_1, " (", input$gr_mult_id_time_range[1], sep="")
                                                table$Period[table$Period_num == 1] <- x
                                        }else{
                                                x <- paste(rv$time_name_1, " (", as.character(rv$time_range_start), sep="")
                                                table$Period[table$Period_num == 1] <- x
                                        }
                                        if(input$gr_mult_id_time_range[2] < rv$time_limit_1){
                                                table$Period[table$Period_num == 1] <- paste(x, "-", input$gr_mult_id_time_range[2], ")", sep="")
                                        }else{
                                                table$Period[table$Period_num == 1] <- paste(x, "-", as.character(rv$time_limit_1), ")",sep="")
                                        }
                                        # Period 2
                                        if(rv$time_subper_num==2){
                                                if(input$gr_mult_id_time_range[1] > rv$time_limit_1+1){
                                                        x <- paste(rv$time_name_2, " (", input$gr_mult_id_time_range[1], sep="")
                                                        table$Period[table$Period_num == 2] <- x
                                                }else{
                                                        x <- paste(rv$time_name_2, " (", as.character(rv$time_limit_1+1), sep="")
                                                        table$Period[table$Period_num == 2] <- x
                                                }
                                                if(input$gr_mult_id_time_range[2] < rv$time_range_end){
                                                        table$Period[table$Period_num == 2] <- paste(x, "-", input$gr_mult_id_time_range[2], ")", sep="")
                                                }else{
                                                        table$Period[table$Period_num == 2] <- paste(x, "-", as.character(rv$time_range_end), ")", sep="")
                                                }
                                        }else{
                                                if(input$gr_mult_id_time_range[1] > rv$time_limit_1+1){
                                                        x <- paste(rv$time_name_2, " (", input$gr_mult_id_time_range[1], sep="")
                                                ...
                                                if(input$gr_mult_id_time_range[2] < rv$time_limit_2){
                                                        table$Period[table$Period_num == 2] <- paste(x, "-", input$gr_mult_id_time_range[2], ")", sep="")
                                                }else{
                                                        table$Period[table$Period_num == 2] <- paste(x, "-", as.character(rv$time_limit_2), ")", sep="")
                                                }
                                        }
                                        ...
                                        # Period 3
                                        if(rv$time_subper_num==3){
                                                if(input$gr_mult_id_time_range[1] > rv$time_limit_2+1){
                                                        x <- paste(rv$time_name_3, " (", input$gr_mult_id_time_range[1], sep="")
                                                        table$Period[table$Period_num == 3] <- x
                                                }else{
                                                        x <- paste(rv$time_name_3, " (", as.character(rv$time_limit_2+1), sep="")
                                                        table$Period[table$Period_num == 3] <- x
                                                }
                                                if(input$gr_mult_id_time_range[2] < rv$time_range_end){
                                                        table$Period[table$Period_num == 3] <- paste(x, "-", input$gr_mult_id_time_range[2], ")", sep="")
                                                }else{
                                                        table$Period[table$Period_num == 3] <- paste(x, "-", as.character(rv$time_range_end), ")", sep="")
                                                }
                                        }else{
                                                if(input$gr_mult_id_time_range[1] > rv$time_limit_2+1){
                                                        x <- paste(rv$time_name_3, " (", input$gr_mult_id_time_range[1], sep="")
                                                ...
                                                if(input$gr_mult_id_time_range[2] < rv$time_limit_3){
                                                        table$Period[table$Period_num == 3] <- paste(x, "-", input$gr_mult_id_time_range[2], ")", sep="")
                                                }else{
                                                        table$Period[table$Period_num == 3] <- paste(x, "-", as.character(rv$time_limit_3), ")", sep="")
                                                }
                                        }

                                        # Period 4
                                        if(rv$time_subper_num==4){
                                                if(input$gr_mult_id_time_range[1] > rv$time_limit_3+1){
                                                        x <- paste(rv$time_name_4, " (", input$gr_mult_id_time_range[1], sep="")
                                                        table$Period[table$Period_num == 4] <- x
                                                }else{
                                                        x <- paste(rv$time_name_4, " (", as.character(rv$time_limit_3+1), sep="")
                                                        table$Period[table$Period_num == 4] <- x
                                                }
                                                if(input$gr_mult_id_time_range[2] < rv$time_range_end){
                                                        table$Period[table$Period_num == 4] <- paste(x, "-", input$gr_mult_id_time_range[2], ")", sep="")
                                                }else{
                                                        table$Period[table$Period_num == 4] <- paste(x, "-", as.character(rv$time_range_end), ")", sep="")
                                                }
                                        }
                                }
                                ...
                                if(rv$time_subper){
                                        ...
                                        color_reactive()
                                        if(input$gr_mult_id_time_subper){
                                                p <- p + scale_fill_brewer(palette=rv$ae_col_palette_mult)
                                        }
        ...
        # SL.4. Line Plots tab ----
                ...
                # SL.4.1. Inputs and data processing ---- 
                        ...
                        observe({
                                x <- input$gr_line_id_ctries 
                                if(is.null(x)) {x <- character(0)}
                                updateSelectInput(
                                        inputId = 'gr_line_highlight_ctry',
                                        choices = x,
                                        selected = rv$countries_ctry )}
                        )
                ...
                # SL.4.2. Create plots ----
                        ...
                        prepped_data_line <- reactive({
                                ...
                                if(! is.null(input$gr_line_id_vars)){
                                        ...
                                        rv$df_all_data %>%
                                                as_tibble() %>%
                                                filter(Country %in% input$gr_line_id_ctries & 
                        ...
                        createUI_line <- function(table) {
                                ...
                                # Plot counter and plot names
                                        x <- rv$counter_line + 1
                                        plot_name <- paste0("plot_line_", as.character(x))
                                ...
                                # Create subperiod variables to include rectangles
                                if(rv$time_subper & input$gr_line_id_time_subper){
                                        
                                        # If 2 subperiods, then there are 3 cases
                                        if(rv$time_subper_num == 2){
                                                # Case 1 
                                                if(input$gr_line_id_time_range[1] < rv$time_limit_1 & input$gr_line_id_time_range[2] > rv$time_limit_1){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_line_id_time_range[1]-0.5, rv$time_limit_1+0.5), 
                                                                Year_end=c(rv$time_limit_1+0.5, input$gr_line_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_1, rv$time_name_2)
                                                        )
                                                }
                                                # Case 2 
                                                if(input$gr_line_id_time_range[2]<=rv$time_limit_1){
                                                        rectangle_text <- data.frame(
                                                ...
                                                # Case 3
                                                if(input$gr_line_id_time_range[1]>=rv$time_limit_1){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_line_id_time_range[1]-0.5), 
                                                                Year_end=c(input$gr_line_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_2)
                                        ...
                                        if(rv$time_subper_num == 3){
                                                # Case 1
                                                if(input$gr_line_id_time_range[1]<rv$time_limit_1 & input$gr_line_id_time_range[2]>rv$time_limit_2){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_line_id_time_range[1]-0.5, rv$time_limit_1+0.5, rv$time_limit_2+0.5), 
                                                                Year_end=c(rv$time_limit_1+0.5, rv$time_limit_2+0.5,input$gr_line_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_1, rv$time_name_2, rv$time_name_3)
                                                        )
                                                }
                                                # Case 2
                                                if(input$gr_line_id_time_range[1]<rv$time_limit_1 & input$gr_line_id_time_range[2]<=rv$time_limit_2 & input$gr_line_id_time_range[2]>rv$time_limit_1){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_line_id_time_range[1]-0.5, rv$time_limit_1+0.5), 
                                                                Year_end=c(rv$time_limit_1+0.5, input$gr_line_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_1, rv$time_name_2)
                                                ...
                                                # Case 3
                                                if(input$gr_line_id_time_range[1]>=rv$time_limit_1 & input$gr_line_id_time_range[1]<rv$time_limit_2 & input$gr_line_id_time_range[2]>rv$time_limit_2){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_line_id_time_range[1]-0.5, rv$time_limit_2+0.5), 
                                                                Year_end=c(rv$time_limit_2+0.5, input$gr_line_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_2, rv$time_name_3)
                                                        )
                                                }
                                                # Case 4
                                                ...
                                                # Case 6
                                                if(input$gr_line_id_time_range[1]>=rv$time_limit_2){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_line_id_time_range[1]-0.5), 
                                                                Year_end=c(input$gr_line_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_3)
                                        ...
                                        if(rv$time_subper_num == 4){
                                                # Case 1
                                                if(input$gr_line_id_time_range[1]<rv$time_limit_1 & input$gr_line_id_time_range[2]>rv$time_limit_3){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_line_id_time_range[1]-0.5, rv$time_limit_1+0.5, rv$time_limit_2+0.5, rv$time_limit_3+0.5), 
                                                                Year_end=c(rv$time_limit_1+0.5, rv$time_limit_2+0.5, rv$time_limit_3+0.5,input$gr_line_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_1, rv$time_name_2, rv$time_name_3, rv$time_name_4)
                                                        )
                                                }
                                                # Case 2
                                                if(input$gr_line_id_time_range[1]<rv$time_limit_1 & input$gr_line_id_time_range[2]<=rv$time_limit_3 & input$gr_line_id_time_range[2]>rv$time_limit_2){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_line_id_time_range[1]-0.5, rv$time_limit_1+0.5, rv$time_limit_2+0.5), 
                                                                Year_end=c(rv$time_limit_1+0.5, rv$time_limit_2+0.5,input$gr_line_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_1, rv$time_name_2, rv$time_name_3)
                                                ...
                                                # Case 3
                                                if(input$gr_line_id_time_range[1]>=rv$time_limit_1 & input$gr_line_id_time_range[1]<rv$time_limit_2 & input$gr_line_id_time_range[2]>rv$time_limit_3){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_line_id_time_range[1]-0.5, rv$time_limit_2+0.5, rv$time_limit_3+0.5), 
                                                                Year_end=c(rv$time_limit_2+0.5, rv$time_limit_3+0.5,input$gr_line_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_2, rv$time_name_3, rv$time_name_4)
                                                        )
                                                }
                                                # Case 4
                                                ...
                                                # Case 5
                                                if(input$gr_line_id_time_range[1]>=rv$time_limit_1 & input$gr_line_id_time_range[1]<rv$time_limit_2 & input$gr_line_id_time_range[2]<=rv$time_limit_3 & input$gr_line_id_time_range[2]>rv$time_limit_2){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_line_id_time_range[1]-0.5, rv$time_limit_2+0.5), 
                                                                Year_end=c(rv$time_limit_2+0.5, input$gr_line_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_2, rv$time_name_3)
                                                ...
                                                # Case 6
                                                if(input$gr_line_id_time_range[1]>=rv$time_limit_2 & input$gr_line_id_time_range[1]<rv$time_limit_3& input$gr_line_id_time_range[2]>rv$time_limit_3){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_line_id_time_range[1]-0.5, rv$time_limit_3+0.5), 
                                                                Year_end=c(rv$time_limit_3+0.5, input$gr_line_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_3, rv$time_name_4)
                                                        )
                                                }
                                                # Case 7
                                                ...
                                                # Case 9
                                                if(input$gr_line_id_time_range[1]>=rv$time_limit_2 & input$gr_line_id_time_range[2]<=rv$time_limit_3){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_line_id_time_range[1]-0.5), 
                                                                Year_end=c(input$gr_line_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_3)
                                                ...
                                                # Case 10
                                                if(input$gr_line_id_time_range[1]>=rv$time_limit_3){
                                                        rectangle_text <- data.frame(
                                                                Year_start=c(input$gr_line_id_time_range[1]-0.5), 
                                                                Year_end=c(input$gr_line_id_time_range[2]+0.5),
                                                                Period=c(rv$time_name_4)
                                ...
                                # Subperiod rectangles, their labels and the dotted lines separating 
                                if(rv$time_subper & input$gr_line_id_time_subper & !all(is.na(table$Value))){
                                        p <- p + 
                                                geom_rect(data = rectangle_text,
                                                        aes(NULL, NULL, xmin=-Inf, xmax=Inf),
                                                        ymin = y_max_new - y_range_new*size_factor ,
