...
# Load necessary dataframes, vectors and lists ----
        ...
        # Color palettes
        x <- filter(brewer.pal.info, colorblind == TRUE)
        ...
        # Default parameters
                ...
                # Countries
                default_countries_ctry <- c("Pakistan")                                                           # Select only one country as analyzed country. For country names check ctry_vec.
                default_countries_str <- c("Bangladesh", "Egypt, Arab Rep.", "Ethiopia", "India")                 # Select structural countries. No limit. Check that there is no overlapping with analyzed country and aspirational countries. For country names check ctry_vec.
                default_countries_asp <- c("Indonesia", "Mexico", "Turkey", "Vietnam")                            # Select aspirational countries. No limit. Check that there is no overlapping with analyzed country and structural countries. For country names check ctry_vec.
                default_countries_reg <- c("South Asia")                                                          # select regions to be included. No limit. For country names check reg_vec.
                default_countries_all <- FALSE
                
                default_time_range_start <- 2000                                                 # Make sure it makes sense with time slider max and min
                default_time_range_end <- 2019                                                   # Make sure it makes sense with time slider max and min
                default_time_subper <- TRUE                                                      # Logical vector with one element defining whether subperiods will be included
                default_time_subper_num <- 3                                                     # Select between 2, 3 or 4
                default_time_limit_1 <- 2008                                                     # Make sure it makes sense with time_range parameters(start and end) and the other subperiod limits
                default_time_limit_2 <- 2014                                                     # Make sure it makes sense with time_range parameters(start and end) and the other subperiod limits
                default_time_limit_3 <- 2018                                                     # Make sure it makes sense with time_range parameters(start and end) and the other subperiod limits
                default_time_name_1 <- "Historical"
                default_time_name_2 <- "Recent past"
                default_time_name_3 <- "Present"
                default_time_name_4 <- "Period 4"
                
                # Variables                                                                     # Find out variable name and code of the variables you want to select (check var_df)
                default_wdi_vars <- c("GDP growth (annual %) | NY.GDP.MKTP.KD.ZG",
                        "Inflation, consumer prices (annual %) | FP.CPI.TOTL.ZG",
                        "GDP per capita, PPP (current international $) | NY.GDP.PCAP.PP.CD")
                
                # Aesthetic parameters
                default_colour_spinner <- c("#009FDA")                                           # Select color. Check options at: https://www.color-hex.com/
                default_colour_graphs <- c("#EC7063")                                            # Select color. Check options at: https://www.color-hex.com/
                default_colour_graphs_line <- c("#FC4024")                                       # Select color. Check options at: https://www.color-hex.com/
                default_colour_palette_mult <- c("Oranges")                                      # Select color. Check options at: https://www.color-hex.com/
                default_selected_font <- c("Calibri")                                            # Select among 4 available fonts: (Calibri = "Calibri", Arial = "sans", Courier = "mono", TimesNewRoman = "serif"). Example: for TimesNewRoman type "serif" instead of "Calibri".
                
                # Calculate auxiliary vectors
                default_countries_selected <- c(default_countries_ctry, default_countries_asp, default_countries_str, default_countries_reg)
                default_countries_selected_iso2c <- ctry_df[ctry_df[ ,"country"] %in% default_countries_selected, "iso2c"]
                default_wdi_vars_code <- filter(var_df, name_indicator %in% default_wdi_vars) %>% pull(indicator)
                default_time_range <- c(default_time_range_start, default_time_range_end)
                
                default_datasets <- c("WDI")
                default_remove_dup <- TRUE
                default_remove_na <- FALSE
                
# Initial country lists
init_available_str <- ctry_vec[!ctry_vec %in% c(default_countries_ctry, default_countries_asp)]
init_available_asp <- ctry_vec[!ctry_vec %in% c(default_countries_ctry, default_countries_str)]
                                        ...
                                        sidebarPanel(
                                                ...
                                                h4("Step 1: Select countries"),
                                                        ...
                                                        pickerInput(
                                                                ...
                                                                selected = default_countries_ctry,
                                                        ...
                                                        selectizeInput(
                                                                ...
                                                                multiple = TRUE,
                                                                selected = default_countries_str),
                                                        ...
                                                        selectizeInput(
                                                                ...
                                                                multiple = TRUE,
                                                                selected = default_countries_asp),
                                                        ...
                                                        conditionalPanel(
                                                                ...
                                                                conditionalPanel(
                                                                        ...
                                                                        numericInput(
                                                                                ...
                                                                                min = 2,
                                                                                max = 4,
                                                                                value = default_time_subper_num),
                                                                        
                                                                        textInput(
                                                                                inputId = "in_id_time_name_1",
                                                                                label="Period 1 name",
                                                                                value = default_time_name_1),
                                                                        
                                                                        textInput(
                                                                                inputId = "in_id_time_name_2",
                                                                                label="Period 2 name",
                                                                                value = default_time_name_2),
                                                                        
                                                                        sliderInput(
                                                                                inputId = "in_id_time_limit_1",
                                                                                label = "Select last year of Period 1",
                                                                                sep = "",
                                                                                min = default_time_range_start,
                                                                                max = default_time_range_end,
                                                                                step = 1,
                                                                                value = default_time_limit_1),
                                        ...
                                        sidebarPanel(
                                                ...
                                                h4("Plot parameters"),
                                                        ...
                                                        sliderInput(
                                                                ...
                                                                max = default_time_range_end,
                                                                step = 1,
                                                                value = c(default_time_range_start,default_time_range_end)
                                                        ),
                                                        hr(),
                                                ...
                                                h4("Display options"),
                                                        ...
                                                        pickerInput(
                                                                ...
                                                                label = "Select font",
                                                                choices = font_list,
                                                                selected = default_selected_font,
                                                                choicesOpt = list(
                                                ...
                                                h4("Display options"),
                                                        ...
                                                        conditionalPanel(
                                                                ...
                                                                colourpicker::colourInput(
                                                                        ...
                                                                        value = default_colour_graphs_line,
                                                                        allowTransparent = TRUE,
                                                                        closeOnClick = TRUE),
                                                        ),
...
server <- function(input, output, session){
        ...
        rv <- reactiveValues(
                ...
                # Countries
                countries_ctry = default_countries_ctry,
                countries_str = default_countries_str,
                countries_asp = default_countries_asp,
                countries_reg = default_countries_reg,
                countries_all = default_countries_all,

                # Time
                time_range_start = default_time_range_start,
                time_range_end = default_time_range_end,
                time_subper = default_time_subper,
                time_subper_num = default_time_subper_num,
                time_limit_1 = default_time_limit_1,
                time_limit_2 = default_time_limit_2,
                time_limit_3 = default_time_limit_3,
                time_name_1 = default_time_name_1,
                time_name_2 = default_time_name_2,
                time_name_3 = default_time_name_3,
                time_name_4 = default_time_name_4,

                # Variables
                wdi_vars = default_wdi_vars,

                # Aesthetic parameters
                ae_colour_spinner = default_colour_spinner,
                ae_colour_graphs = default_colour_graphs,
                ae_colour_graphs_line = default_colour_graphs_line,
                ae_colour_palette_mult = default_colour_palette_mult,
                ae_selected_font = default_selected_font,

                # Auxiliary vectors
                countries_selected = default_countries_selected,
                countries_selected_iso2c = default_countries_selected_iso2c,
                wdi_vars_code = default_wdi_vars_code,
                time_range = default_time_range,
                
                # Datasets
                datasets = default_datasets,
                remove_dup = default_remove_dup,
                remove_na = default_remove_dup,
                ...
                # Dataframes
                df_wdi = empty_data,
                df_external_last = empty_data %>% select(),
                df_external_all = empty_data,
                df_all_data_raw = empty_data,
                df_all_data = empty_datatable,
                ...
                # SL.1.1. Inputs and data processing ----
                        ...
                        ## SL.1.1.1. Countries choice: update countries available in menus (UI.1.1.1.) ====
                                ...
                                fun_listen_countries <- reactive({
                                        no_changes <- as.logical(c(
                                                setequal(rv$countries_ctry, input$in_id_countries_ctry),
                                                setequal(rv$countries_str, input$in_id_countries_str),
                                                setequal(rv$countries_asp, input$in_id_countries_asp),
                                                setequal(rv$countries_reg, input$in_id_countries_reg),
                                                setequal(rv$countries_all, input$in_id_countries_all)
                                                ))
                        ...
                        observeEvent(input$in_id_reset_confirmation, {
                                if(input$in_id_reset_confirmation){
                                        ...
                                        # Data
                                        rv$df_wdi <- empty_data
                                        rv$external_last <- empty_data %>% select()
                                        rv$external_all <- empty_data
                                        rv$df_all_data_raw <- empty_data
                                        rv$df_all_data <- empty_datatable
                                        ...
                                        rv$time_range <- c(default_time_range_start, default_time_range_end)
                ...
                # SL.2.1. Inputs and data processing ----
                        ...
                        observeEvent(to_listen_data(),{
                                ...
                                flags_bar$Ctry_group <- ifelse(
                                        flags_bar$country %in% rv$countries_ctry , "Analized", 
        ...
        # SL.3. Multiple Countries Bar Plot tab ----
                ...
                # SL.3.1. Inputs and data processing ---- 
                        ...
                        output$gr_mult_id_color_ui <- renderUI({
                                pickerInput(
                                        ...
                                        selected = rv$ae_colour_palette_mult,
                                        multiple = FALSE
                                ) %>%
                                        shinyInput_label_embed(
                                                shiny_iconlink() %>%
                        ...
                        color_reactive <- eventReactive(input$gr_mult_id_color_pal,{
                                rv$ae_colour_palette_mult <- input$gr_mult_id_color_pal
                                }
                        )
                ...
                # SL.3.2. Create plots ----
                        ...
                        createUI_mult <- function(table) {
                                ...
                                color_reactive()
                                if(input$gr_mult_id_time_subper){
                                        p <- p + scale_fill_brewer(palette=rv$ae_colour_palette_mult)
                                }
